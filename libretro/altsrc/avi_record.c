...
static bool	Avi_RecordVideoStream_BMP ( RECORD_AVI_PARAMS *pAviParams )
{
	...
	pBitmapIn = sdlscrn->pixels + 2*1024*(retroh-1) ;

	for ( y=0 ; y<480; y++ )
	{
		
		pBitmapOut = LineBuf;
		PixelConvert_rgb565to24Bits_BGR(LineBuf,(Uint16 *)pBitmapIn, 640);

		 if ( (int)fwrite ( pBitmapOut , 1 , retrow *3 , pAviParams->FileOut ) != retrow *3 )
		{
			perror ( "Avi_RecordVideoStream_BMP" );
			Log_AlertDlg ( LOG_ERROR, "AVI recording : failed to write bmp video frame" );
			return false;
		}
		pBitmapIn -= 1024*2;			/* go from bottom to top */
